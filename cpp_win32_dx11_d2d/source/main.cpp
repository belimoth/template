...
void app_graphics_device_and_swap_chain() {
	...
	D2D1CreateFactory( D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof( ID2D1Factory1 ), &options, (void**) &d2d.f );
	d2d.f->CreateDevice( dxgi.d, &d2d.d );
	d2d.d->CreateDeviceContext( D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &d2d_context );
}
...
void app_graphics_rtv() {
	...
	IDXGISurface *dxgi_s;
	app.graphics.rtb->QueryInterface( __uuidof( IDXGISurface ), (void**) &dxgi_s );

	D2D1_BITMAP_PROPERTIES1 bp;
	bp.pixelFormat   = { DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_IGNORE };
	bp.dpiX          = 0.0;
	bp.dpiY          = 0.0;
	...
	d2d_context->CreateBitmapFromDxgiSurface( dxgi_s, &bp, &d2dTarget );
	d2d_context->SetTarget( d2dTarget );
}
...
void app_graphics() {
    app_graphics_device_and_swap_chain();
	app_graphics_rtv();
	app_graphics_dsv();
